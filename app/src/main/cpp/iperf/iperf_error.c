...
iperf_strerror(int int_errno) {
    ...
    switch (int_errno) {
        ...
        case IEDURATION:
            ...
            break;
        case IENUMSTREAMS:
            snprintf(errstr, len, "number of parallel streams too large (maximum = %d)",
                     MAX_STREAMS);
            break;
        case IEBLOCKSIZE:
            ...
            break;
        case IEBUFSIZE:
            snprintf(errstr, len, "socket buffer size too large (maximum = %d bytes)",
                     MAX_TCP_BUFFER);
            break;
        case IEINTERVAL:
            snprintf(errstr, len, "invalid report interval (min = %g, max = %g seconds)",
        ...
        case IEBIND: /* UNUSED */
            ...
            break;
        case IEUDPBLOCKSIZE:
            snprintf(errstr, len, "block size invalid (minimum = %d bytes, maximum = %d bytes)",
        ...
        case IESETCLIENTAUTH:
            snprintf(errstr, len,
                     "you must specify username (max 20 chars), password (max 20 chars) and a path to a valid public rsa client to be used");
            break;
        case IESETSERVERAUTH:
            snprintf(errstr, len,
                     "you must specify path to a valid private rsa server to be used and a user credential file");
            break;
        case IEBADFORMAT:
