...
iperf_udp_recv(struct iperf_stream *sp) {
    ...
    if (sp->test->state == TEST_RUNNING) {
        ...
        if (pcount >= sp->packet_count + 1) {
            ...
            if (sp->test->debug)
                fprintf(stderr,
                        "OUT OF ORDER - incoming packet sequence %" PRIu64 " but expected sequence %d on stream %d",
...
iperf_udp_accept(struct iperf_test *test) {
     ...
     */
    len = sizeof(sa_peer);
    if ((sz = recvfrom(test->prot_listener, &buf, sizeof(buf), 0, (struct sockaddr *) &sa_peer,
                       &len)) < 0) {
     ...
     */
    test->prot_listener = netannounce(test->settings->domain, Pudp, test->bind_address,
                                      test->server_port);
...
iperf_udp_listen(struct iperf_test *test) {
    ...
    if ((s = netannounce(test->settings->domain, Pudp, test->bind_address, test->server_port)) <
        0) {
        i_errno = IESTREAMLISTEN;
        return -1;
    }
...
iperf_udp_connect(struct iperf_test *test) {
    ...
    if ((s = netdial(test->settings->domain, Pudp, test->bind_address, test->bind_port,
